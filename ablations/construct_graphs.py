...
def add_noise_to_embeddings(x, device, feature_noise_level = 0.0, node_noise_level = 0.0):
...
def construct_PyG_graph_from_LLM(
    ...
    heads : int,
    #
    ablation_operation : int, # 0 = None, 1 = Random, 2 = Unitary weights, 3 = Single unit weight, 4 = random attentions (prior to construction)
    feature_noise_level : float,
    node_noise_level : float,
    #
    device
  ):
  ...
  while s < document_length - 1:
    ...
    if ablation_operation == 4:
      chunk_attention_T = torch.rand_like(chunk_attention_T)
  ...
  if ablation_operation == 1:
    edge_attr_aggregated = torch.rand_like(edge_attr_aggregated)
  elif ablation_operation == 2:
    edge_attr_aggregated = torch.ones_like(edge_attr_aggregated)
  elif ablation_operation == 3:
    edge_attr_aggregated = torch.ones(edge_attr_aggregated.size(0), 1)
    
  if feature_noise_level > 0 and node_noise_level > 0:
    node_attr_aggregated_without_isolated_nodes = add_noise_to_embeddings(
