...
STORAGE_PATH = '/scratch/ssd004/scratch/pimentel/DynamicCOO/ablation_outputs/'
  ...
  try:
    ...
    for i, row in training_df.iterrows():
      training_graphs_pyg.append(
        construct_graphs.construct_PyG_graph_from_LLM(
          ...
          heads = HEADS,
          #
          ablation_operation = ABLATION_OPERATION,
          node_noise_level = NODE_ABLATION_NOISE_LEVEL,
          feature_noise_level = NODE_ABLATION_FEATURE_NOISE_LEVEL,
          #
          device = DEVICE
        )
      )
    ...
    if GNN == 'GAT':
      model = models.GAT(
        ...
        edge_dimension = AGGREGATION_LEVELS_FEATURE_COUNT[AGGREGATION_LEVEL] if ABLATION_OPERATION != 3 else 1,
    ...
    if not evaluate_test:
      ...
      pd.DataFrame({
        ...
        os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], ABLATION_LEVELS[ABLATION_OPERATION], f'{NODE_ABLATION_NOISE_LEVEL}-{NODE_ABLATION_FEATURE_NOISE_LEVEL}', f'{trial_number}', 'metrics.csv'),
        mode = 'a',
        header = not os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], ABLATION_LEVELS[ABLATION_OPERATION], f'{NODE_ABLATION_NOISE_LEVEL}-{NODE_ABLATION_FEATURE_NOISE_LEVEL}', f'{trial_number}', 'metrics.csv')),
        index = False
      )

      os.makedirs(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], ABLATION_LEVELS[ABLATION_OPERATION], f'{NODE_ABLATION_NOISE_LEVEL}-{NODE_ABLATION_FEATURE_NOISE_LEVEL}', f'{trial_number}', f'{random_state}'), exist_ok = True)
      ...
      }).to_csv(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], ABLATION_LEVELS[ABLATION_OPERATION], f'{NODE_ABLATION_NOISE_LEVEL}-{NODE_ABLATION_FEATURE_NOISE_LEVEL}', f'{trial_number}', f'{random_state}', 'predictions.csv'), index = False)

      pd.DataFrame({
        'epoch_runtime' : epoch_runtimes,
      }).to_csv(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], ABLATION_LEVELS[ABLATION_OPERATION], f'{NODE_ABLATION_NOISE_LEVEL}-{NODE_ABLATION_FEATURE_NOISE_LEVEL}', f'{trial_number}', f'{random_state}', 'runtimes.csv'), index = False)
...
def objective_function(trial):
  ...
  if CHUNKING:
    ...
    left_stride = trial.suggest_categorical('left_stride', [0, 32, 64, 128, 256])
    right_stride = trial.suggest_categorical('right_stride', [0, 32, 64, 128, 256])
  else:
    # chunk_size = MAXIMUM_SEQUENCE_LENGTH
    left_stride = 0
    right_stride = 0

  attention_pooling_operation = trial.suggest_categorical('attention_pooling_operation', ['min', 'mean', 'max']) # ['min', 'mean', 'sum', 'max']
  embedding_pooling_operation = trial.suggest_categorical('embedding_pooling_operation', ['min', 'mean', 'max']) # ['min', 'mean', 'sum', 'max']
  threshold = trial.suggest_float('threshold', MINIMUM_THRESHOLD, MAXIMUM_THRESHOLD)
  #
  ...
  hidden_dimension = trial.suggest_int('hidden_dimension', 32, 256)
  number_of_hidden_layers = trial.suggest_int('number_of_hidden_layers', 0, 4)
  dropout_rate = trial.suggest_float('dropout_rate', 0.3, 0.6)
  global_pooling = trial.suggest_categorical('global_pooling', ['mean', 'sum', 'max'])
  ...
  plateau_patience = trial.suggest_int('plateau_patience', 10, 25)
  ...
  validation_performance, validation_loss, training_loss, epoch = train_and_predict(
    ...
    left_stride = left_stride,
    right_stride = right_stride,
    attention_pooling_operation = attention_pooling_operation,
    embedding_pooling_operation = embedding_pooling_operation,
    threshold = threshold,
    #
...
if __name__ == '__main__':
  ...
  parser.add_argument('--ablation_operation', required = True, type = int, help = 'Which ablation to apply to the attention coefficients: 0 (no ablation), 1 (random weights), 2 (unitary weights), 3 (single unitary weight), or 4 (random attentions, i.e., prior to construction).')
  parser.add_argument('--node_ablation_noise_level', required = True, type = float, help = 'Percentage of nodes in each graph to be affected by noise.')
  parser.add_argument('--node_ablation_feature_noise_level', required = True, type = float, help = 'Percentage of features in each node node embedding from each graph to be affected by noise.')
  ...
  ABLATION_OPERATION = args.ablation_operation
  NODE_ABLATION_NOISE_LEVEL = args.node_ablation_noise_level
  NODE_ABLATION_FEATURE_NOISE_LEVEL = args.node_ablation_feature_noise_level
  ...
  if ABLATION_OPERATION not in [0, 1, 2, 3, 4]:
    raise ValueError('The ablation operation must be 0 (no ablation), 1 (random weights), 2 (unitary weights), 3 (single unitary weight), or 4 (random attentions, i.e., prior to construction).')
  
  if NODE_ABLATION_NOISE_LEVEL < 0 or NODE_ABLATION_NOISE_LEVEL > 1 or NODE_ABLATION_FEATURE_NOISE_LEVEL < 0 or NODE_ABLATION_FEATURE_NOISE_LEVEL > 1:
    raise ValueError('The noise lebels must be between 0 and 1.')
  ...
  ABLATION_LEVELS = ['No_Ablation', 'Random_Weights', 'Unitary_Weights', 'Single_Unitary_Weight', 'Random_Attentions']
  
  study_name = f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}-{"Surrogate" if SURROGATE else "Grouped"}-{AGGREGATION_LEVELS[AGGREGATION_LEVEL]}-{ABLATION_LEVELS[ABLATION_OPERATION]}-{NODE_ABLATION_NOISE_LEVEL}-{NODE_ABLATION_FEATURE_NOISE_LEVEL}'
  ...
  for _, trial in top_N_trials.iterrows():
    ...
    os.makedirs(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], ABLATION_LEVELS[ABLATION_OPERATION], f'{NODE_ABLATION_NOISE_LEVEL}-{NODE_ABLATION_FEATURE_NOISE_LEVEL}', f'{trial["number"]}'), exist_ok = True)
    ...
    for random_state in range(SEED - NUMBER_OF_TEST_RUNS // 2, SEED + NUMBER_OF_TEST_RUNS // 2):
      ...
      if os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], ABLATION_LEVELS[ABLATION_OPERATION], f'{NODE_ABLATION_NOISE_LEVEL}-{NODE_ABLATION_FEATURE_NOISE_LEVEL}', f'{trial["number"]}', f'{random_state}', 'predictions.csv')) and os.path.isfile(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], ABLATION_LEVELS[ABLATION_OPERATION], f'{NODE_ABLATION_NOISE_LEVEL}-{NODE_ABLATION_FEATURE_NOISE_LEVEL}', f'{trial["number"]}', f'{random_state}', 'runtimes.csv')):
        
        predictions = pd.read_csv(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], ABLATION_LEVELS[ABLATION_OPERATION], f'{NODE_ABLATION_NOISE_LEVEL}-{NODE_ABLATION_FEATURE_NOISE_LEVEL}', f'{trial["number"]}', f'{random_state}', 'predictions.csv'))
