  ...
  try:
    ...
    if not evaluate_test:
      ...
      pd.DataFrame({
        ...
        os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], f'{trial_number}', 'metrics.csv'),
        mode = 'a',
        header = not os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], f'{trial_number}', 'metrics.csv')),
        index = False
      )

      os.makedirs(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], f'{trial_number}', f'{random_state}'), exist_ok = True)
      ...
      }).to_csv(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], f'{trial_number}', f'{random_state}', 'predictions.csv'), index = False)

      pd.DataFrame({
        'epoch_runtime' : epoch_runtimes,
      }).to_csv(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], f'{trial_number}', f'{random_state}', 'runtimes.csv'), index = False)
...
if __name__ == '__main__':
  ...
  for _, trial in top_N_trials.iterrows():
    ...
    os.makedirs(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], f'{trial["number"]}'), exist_ok = True)
    ...
    for random_state in range(SEED - NUMBER_OF_TEST_RUNS // 2, SEED + NUMBER_OF_TEST_RUNS // 2):
      ...
      if os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], f'{trial["number"]}', f'{random_state}', 'predictions.csv')) and os.path.isfile(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], f'{trial["number"]}', f'{random_state}', 'runtimes.csv')):
        
        predictions = pd.read_csv(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', AGGREGATION_LEVELS[AGGREGATION_LEVEL], f'{trial["number"]}', f'{random_state}', 'predictions.csv'))
