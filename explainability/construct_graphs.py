...
def construct_PyG_graph_from_LLM(
  ...
  if surrogate:
    ...
    from_ids = [mapping[x] for x in edge_index_unique_PyG_without_isolated_nodes[0].cpu().numpy()]
    from_tokens = tokenizer.convert_ids_to_tokens(from_input_identifiers)
    from_PyG_identifiers = edge_index_unique_PyG_without_isolated_nodes[0].cpu().numpy()
    to_ids = [mapping[x] for x in edge_index_unique_PyG_without_isolated_nodes[1].cpu().numpy()]
    to_tokens = tokenizer.convert_ids_to_tokens(to_input_identifiers)
    to_PyG_identifiers = edge_index_unique_PyG_without_isolated_nodes[1].cpu().numpy()
  else:  
    from_ids = [mapping[x] for x in edge_index_unique_PyG_without_isolated_nodes[0].cpu().numpy()]
  ...
  token_and_identifiers = pd.Series(list(zip(from_PyG_identifiers, from_tokens)) + list(zip(to_PyG_identifiers, to_tokens))).drop_duplicates()
  # unique_ids = [x[0] for x in token_and_identifiers]
  unique_tokens = [x[1] for x in token_and_identifiers]
  ...
  document_graph_PyG = torch_geometric.data.Data(
    ...
    # Explainability-based properties
    from_ids = from_ids,
    from_tokens = from_tokens,
    from_PyG_identifiers = from_PyG_identifiers,
    to_ids = to_ids,
    to_tokens = to_tokens,
    to_PyG_identifiers = to_PyG_identifiers,
    node_tokens = unique_tokens
  ).to(device)
