...
def get_values_per_document_distribution(
  ...
  os.makedirs(os.path.join('..', 'data', f'{LARGE_LANGUAGE_MODEL.replace("/", "-")}-{"Surrogate" if SURROGATE else "Grouped"}'), exist_ok = True)
  ...
  for i, row in training_df.iterrows():
    ...
    total_values_for_edges = edges * AGGREGATION_LEVELS_FEATURE_COUNT[AGGREGATION_LEVEL]
  ...
  pd.DataFrame(l, columns = ['split', 'index', 'nodes', 'total_values_for_nodes', 'edges', 'total_values_for_edges', 'total_values_for_graph']) \
    .to_csv(os.path.join('..', 'data', f'{LARGE_LANGUAGE_MODEL.replace("/", "-")}-{"Surrogate" if SURROGATE else "Grouped"}', f'{DATASET}.csv'), index = False)
