  ...
  try:
    ...
    for i, row in training_df.iterrows():
      training_graphs_pyg.append(
        construct_graphs.construct_PyG_graph_from_LLM_using_sliding_windows(
          text = row['text'],
          label = row['label'],
          split = 'train',
          index = i,
    ...
    if not evaluate_test:
      ...
      pd.DataFrame({
        ...
        os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', 'Sliding_Windows', f'{trial_number}', 'metrics.csv'),
        mode = 'a',
        header = not os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', 'Sliding_Windows', f'{trial_number}', 'metrics.csv')),
        index = False
      )

      os.makedirs(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', 'Sliding_Windows', f'{trial_number}', f'{random_state}'), exist_ok = True)
      ...
      }).to_csv(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', 'Sliding_Windows', f'{trial_number}', f'{random_state}', 'predictions.csv'), index = False)

      pd.DataFrame({
        'epoch_runtime' : epoch_runtimes,
      }).to_csv(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', 'Sliding_Windows', f'{trial_number}', f'{random_state}', 'runtimes.csv'), index = False)
...
def objective_function(trial):
  ...
  co_occurrence_pooling_operation = trial.suggest_categorical('co_occurrence_pooling_operation', ['sum']) # ['min', 'mean', 'sum', 'max']
  ...
  validation_performance, validation_loss, training_loss, epoch = train_and_predict(
    ...
    left_stride = left_stride,
    right_stride = right_stride,
    co_occurrence_pooling_operation = co_occurrence_pooling_operation,
...
if __name__ == '__main__':
  ...
  _, _, _, TOKENIZER, LLM, MAXIMUM_SEQUENCE_LENGTH, EMBEDDING_OUTPUT_KEY, _ = utils.load_feature_extractor(LARGE_LANGUAGE_MODEL)
  LLM.to(DEVICE)
  ...
  for _, trial in top_N_trials.iterrows():
    ...
    os.makedirs(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', 'Sliding_Windows', f'{trial["number"]}'), exist_ok = True)
    ...
    for random_state in range(SEED - NUMBER_OF_TEST_RUNS // 2, SEED + NUMBER_OF_TEST_RUNS // 2):
      ...
      if os.path.exists(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', 'Sliding_Windows', f'{trial["number"]}', f'{random_state}', 'predictions.csv')) and os.path.isfile(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', 'Sliding_Windows', f'{trial["number"]}', f'{random_state}', 'runtimes.csv')):
        
        predictions = pd.read_csv(os.path.join(STORAGE_PATH, f'{DATASET}-{GNN}-{LARGE_LANGUAGE_MODEL.replace("/", "-")}', 'Surrogate' if SURROGATE else 'Grouped', 'Sliding_Windows', f'{trial["number"]}', f'{random_state}', 'predictions.csv'))
